...
import { Search, Filter, MoreHorizontal, Code, ExternalLink, Trash2, Library } from 'lucide-react';
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10 lg:py-12">
      <div className="flex flex-col gap-8">
        ...
        {components.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 border-2 border-dashed rounded-3xl bg-muted/10">
            <Library className="size-12 text-muted-foreground mb-4" />
            <h3 className="text-lg font-semibold">No primitives found</h3>
            <p className="text-muted-foreground mb-6">Upload code or designs to start building your library.</p>
            <Link to="/app/upload">
              ...
              <Card key={comp.id} className="overflow-hidden group hover:ring-2 hover:ring-primary/20 transition-all">
                ...
                <CardContent className="pt-4">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="font-semibold truncate pr-2">{comp.name}</h3>
                    <Badge variant="secondary" className="text-[10px] uppercase shrink-0">{comp.category}</Badge>
                  </div>
                ...
                <CardFooter className="pt-0 flex justify-between">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem className="text-destructive" onClick={() => removeComponent(comp.id)}>
